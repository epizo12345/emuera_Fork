...
{
    private static int RunR0F4D1(string root, string mode)
    {
        root = Path.GetFullPath(root);
        if (!Path.GetFileName(root).StartsWith("CompactReplacementR0F4D1_", StringComparison.Ordinal)
        ...
        try
        {
            ...
            if (!DifferentialDeterminism.Configure(true, NextRuntimeDifferentialSeed,
                ...
                throw new InvalidOperationException("R0-F4D1 deterministic setup failed");
            ...
            using (var log = new StreamWriter(new FileStream(Path.Combine(root, "initialize.log"), FileMode.CreateNew)))
                if (!process.Initialize(log).GetAwaiter().GetResult())
                    throw new InvalidOperationException("R0-F4D1 initialization failed");
            B1Proof.Active = false;
            Preload.Clear();

            var result = process.RunR0F4D1(candidate, root);
            if (window.Visible || NextRuntimeMode || NextRuntimeDifferentialCapturePath is not null)
                throw new InvalidOperationException("R0-F4D1 GUI/Next isolation failed");
