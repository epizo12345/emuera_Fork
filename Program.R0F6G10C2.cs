...
{
    private static int RunR0F6G10C2(string root, string outputPath, string mode)
    {
        root = Path.GetFullPath(root);
        outputPath = Path.GetFullPath(outputPath);
        var legacy = mode == "LegacyLoadMenu";
        if (!IsCompactProductionProofRoot(root) || File.Exists(outputPath) ||
            mode is not ("LegacyLoadMenu" or "CompactLoadMenu" or "CompactLoadD9" or "CompactLoadH" or "CompactNewGame" or "MetadataCrossCheck")) return 64;
        try
        {
            ...
            if (!DifferentialDeterminism.Configure(true, NextRuntimeDifferentialSeed,
                ...
                throw new InvalidOperationException("R0-F6G10C2 deterministic setup failed");
            ...
            using (var log = new StreamWriter(new FileStream(Path.ChangeExtension(outputPath, ".init.log"), FileMode.CreateNew)))
                if (!process.Initialize(log).GetAwaiter().GetResult())
                    throw new InvalidOperationException("R0-F6G10C2 initialization failed");
            Preload.Clear();
            B1Proof.WriteJson(outputPath, process.R0F6G10C2Run(mode));
            return 0;
        }
        catch (Exception ex)
        {
