...
# 配布フォルダについて
『配布/Emuera.NET_ゲームパッド対応_v1』には、2026-08-16時点の最新mainの最適化とController/Gamepad対応を統合したRelease Publish単一EXEを置いています。ソース側の標準操作は、× / Aが決定、○ / Bが戻る・キャンセル、△ / YがESC・マウス右クリック相当の高速送り、□ / Xがゲームパッド設定を開く（初期設定。変更可能）、D-pad / Left Stickが移動、LB / RBがページ移動、OPTIONS / Startは初期未割り当てです。
...
- 変更したソースはGitの現行ブランチで管理しています。旧差分ZIPは現在の正式配布物ではありません。
