...
{
    private static int RunR0F4E1Break(string root, string mode)
    {
        root = Path.GetFullPath(root);
        if (!Path.GetFileName(root).StartsWith("CompactReplacementR0F4E1_", StringComparison.Ordinal)
            || mode is not ("control" or "candidate")) return 64;
        var output = Path.Combine(root, "break-result.json");
        var failure = Path.Combine(root, "break-failure.txt");
        ...
        try
        {
            ...
            if (!DifferentialDeterminism.Configure(true, NextRuntimeDifferentialSeed,
                ...
                throw new InvalidOperationException("R0-F4E1 break deterministic setup failed");
            ...
            if (candidate) Preload.Load(Config.GetFiles(ErbDir, "*.ERH").Select(path => path.Value)).GetAwaiter().GetResult();
            ...
            using (var log = new StreamWriter(new FileStream(Path.Combine(root, "break-initialize.log"), FileMode.CreateNew)))
                if (!process.Initialize(log).GetAwaiter().GetResult())
                    throw new InvalidOperationException("R0-F4E1 break initialization failed\n" + window.R0B1Console.GetLog());
            B1Proof.Active = false;
            Preload.Clear();
            var result = process.RunR0F4E1BreakMatrix(candidate);
            if (window.Visible || NextRuntimeMode || NextRuntimeDifferentialCapturePath is not null)
                throw new InvalidOperationException("R0-F4E1 break GUI/Next isolation failed");
