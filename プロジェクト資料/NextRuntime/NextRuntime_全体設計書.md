...
R2のProduction VM変更は、ordinal semantic identity、EffectiveName unknown contract、16-byte catalog entry、OpenFrame専用loop ownership、warning/fatal structural classification、Legacy partial SIF set、InvalidStructure StopReason、retained/performance boundaryに限定した。Legacy Runtime production、loader/parser semantics、SourceIndex、PrototypeCompiler、正式EXEは変更していない。

Catalog entryは`CatalogSourceRef 8 bytes + EffectiveNameId 4 bytes + PackedMetadata 4 bytes`である。PackedMetadataはCodeAvailable、EffectiveNameKnown、FunctionKind、必要flagsを保持する。SourceIndexのfile/function配列からphysical name/spanを引き、permanent `Dictionary<SemanticFunctionKey,int>`、FunctionId field、SourceSpan/PhysicalNameId duplicationを持たない。FindByNameはEffectiveNameKnown=falseを候補へ追加しない。

Loop linkerはBREAK/CONTINUE発見時にnearest loop OpenFrameの`LoopControlRecordIndices`へ登録し、StructuralLinkRecord.LoopIndexにはtemporary ownerを保存しない。loop closeは当該frameのリストだけをfinal LoopDescriptor indexへrewriteする。synthetic outer/inner、異種loop、depth3、siblingsのowner testsを含むSelfTest=45/45である。IF/SELECT orderingではwarning-only caseをlinked pendingのまま保持し、fatal close/cross/missing/外側制御はInvalidStructureとした。SIF partial setは`Runtime/Script/Statements/FunctionIdentifier.cs`のPARTIAL/IsPartialと`Runtime/Script/Statements/Instraction.Child.cs`のSIF parserをsource authorityとした。

再監査のraw値はdefinitions=134652、compiled=59103、instructionCount=191273、CALL=12254、JUMP=15、resolved=12269、missing=0、wrongKind=0、CodeAvailable target true/false=1527/10742、operandSpanMismatch=0、IFGroups/Clauses=11051/29189、SELECTGroups/Cases=2825/6415、LoopDescriptors=128、MaxLoopDepth=2、real ExecutableReady=0である。StartLineはdiagnostic専用で、ExactPhysicalStartLineMatch=134649、KnownPositionResidual=3をTSV化した。ordinal identityのraw manifestはSourceIndex preprocessor定義3件とLegacy inline-brace定義3件のfile内件数差を示し、SemanticIdentityMissing/Extra=3/3、EffectiveNameUnknown=3となる。これは禁止されたphysical-name fallbackをしない結果であり、R2必須の0/0・Unknown=0を満たさないため、値を補正せずHOLDとした。
