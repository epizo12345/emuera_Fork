...
通常構成 `E:\GAME-2\テスト版\eramegaten_p` と、大量口上構成 `E:\GAME-2\テスト版\eramegaten_p_口上有り` の起動時間差を、ERB解釈を変えずに実測で分解する。対象は `PrimaryParse` と `ScriptParse` だけであり、高速化実装はこの Phase には含めない。
