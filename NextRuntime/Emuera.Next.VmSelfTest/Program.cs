...
var tests = new List<(string Name, Action Run)>
{
    ...
    ("runtime PRINTL emits ordered text and newline", () => { var p = Executable(ControlLinker.Link(Catalog("X"), [RPO(0, [PrototypeOpcode.PRINTL, PrototypeOpcode.RETURN], ["hello", ""]) ], runtimeStatements: true).Program); var effects = new RecordingEffects(); Check(new VmMachine(p, runtimeEffects: effects).Run(new(0)) == VmStopReason.Returned && effects.Events.SequenceEqual(["text:hello", "newline"])); }),
