...
static int SelfTest()
{
    ...
    try
    {
        ...
        SourceFileIndex IndexHeader(string header)
        {
            var p = Path.Combine(root, "metadata-header-" + tests.Count + ".ERB");
            WriteBom(p, header + "\r\nPRINT 1\r\n");
            return ErbSourceIndexer.IndexFile(p);
        }
        FunctionRuntimeMetadata CompileRuntimeHeader(string header)
        {
            var indexedHeader = IndexHeader(header);
        ...
        tests.Add(("semantic payload is optional for zero operands", () => { var normal = compiler.TryCompile(indexed, function).Function!; var semantic = new FunctionCompiler(semanticEnvironment).TryCompile(indexed, function).Function!; var legacyHeaderPath = Path.Combine(root, "legacy-arg-header.ERB"); WriteBom(legacyHeaderPath, "@GET_USEABLE_EXTRA_TITLE, ARG = 98\r\n#FUNCTION\r\nPRINT 1\r\n"); var legacyHeader = ErbSourceIndexer.IndexFile(legacyHeaderPath); var runtime = new FunctionCompiler(semanticEnvironment).TryCompileRuntime(legacyHeader, legacyHeader.Functions.Single()); Assert(normal.SemanticPayload is null && semantic.SemanticPayload is null && runtime.Status == CompileStatus.Compiled && runtime.Function!.RuntimeMetadata!.Parameters.Length == 1 && runtime.Function.RuntimeMetadata.Parameters[0].Name == "ARG" && runtime.Function.RuntimeMetadata.Parameters[0].Type == RuntimeMetadataValueType.Integer); }));
        tests.Add(("ParenthesizedHeaderMetadataTest", () => { var noArgs = IndexHeader("@A"); var parenthesized = IndexHeader("@B(ARG)"); Assert(noArgs.Functions.Single().Flags == SourceIndexFlags.None && (parenthesized.Functions.Single().Flags & SourceIndexFlags.FunctionMetadata) != 0); var metadata = CompileRuntimeHeader("@B(ARG)"); Assert(metadata.Parameters.Length == 1 && metadata.Parameters[0].Name == "ARG" && metadata.Parameters[0].Type == RuntimeMetadataValueType.Integer); }));
        ...
        tests.Add(("P3L FunctionSourceSession metrics integration", (Action)(() =>
        {
            SourceReaderMetrics.Reset();
            var metricsPath = Path.Combine(root, "p3l-metrics-integration.ERB");
            WriteBom(metricsPath, "@METRICS\r\nPRINT 1\r\n");
            var metricsIndex = ErbSourceIndexer.IndexFile(metricsPath);
            using (var session = FunctionSourceReader.OpenFile(metricsIndex))
            {
        ...
        tests.Add(("P3Y OptionalIntExpression post-preprocess split attribution", (Action)(() =>
        {
            CompileRuntimeMetrics.Reset();
            var options = new CompilerCompatibilityOptions(true, true, true, false);
            var macros = MacroCatalog.FromHeaderText("#DEFINE ONE 1", options);
            var environment = new StructuralSemanticEnvironment(options, macros);
            var p = Path.Combine(root, "p3y-payload-build.ERB");
            WriteBom(p, "@P3Y_PAYLOAD_BUILD\r\nSIF\r\nSIF 1+2\r\nSIF ONE\r\n");
            var f = ErbSourceIndexer.IndexFile(p);
            var result = new FunctionCompiler(environment).TryCompileRuntime(f, f.Functions.Single());
            var metrics = CompileRuntimeMetrics.Snapshot();
            var cohorts = metrics.OptionalIntExpressionCohorts;
        ...
        {
            CompileRuntimeMetrics.Reset();
            var p = Path.Combine(root, "p3y-payload-exception.ERB");
            WriteBom(p, "@P3Y_PAYLOAD_EXCEPTION\r\nSIF (\r\n");
            var f = ErbSourceIndexer.IndexFile(p);
            var result = new FunctionCompiler(semanticEnvironment).TryCompileRuntime(f, f.Functions.Single());
            var cohorts = CompileRuntimeMetrics.Snapshot().OptionalIntExpressionCohorts;
            Assert(result.Status == CompileStatus.Unsupported && cohorts.PreprocessBoundaryTimestampCount == 1 && cohorts.PayloadBuildBoundaryTimestampCount == 0 && cohorts.ParseArenaThroughRecord.Count == 0 && cohorts.PayloadBuildThroughSemanticEnd.Count == 0);
            CompileRuntimeMetrics.Reset();
            var reset = CompileRuntimeMetrics.Snapshot().OptionalIntExpressionCohorts;
            Assert(reset.PreprocessBoundaryTimestampCount == 0 && reset.PayloadBuildBoundaryTimestampCount == 0 && reset.ParseArenaThroughRecord.Count == 0 && reset.PayloadBuildThroughSemanticEnd.Count == 0);
        })));
#endif
