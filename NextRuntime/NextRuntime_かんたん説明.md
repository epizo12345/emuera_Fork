...
軽い目次の答え合わせが終わり、現在は目次から必要な関数だけ取り出して、新しい小さい形式へ変換する試作を行っています。関数全体ではなく、Source Indexが示すphysical byte spanだけを読みます。

まだ新しいVMでゲームを動かしていません。0B-R2では旧Emueraの実際の読み込み処理と目次を答え合わせし、134,652件を安全範囲とfallback範囲に分けて予期しない差を0件にしました。1Aでは実ゲームのcompiler eligible 59,435件のうち54,200件をprototype compileできました。未対応構文は安全にLegacyへfallbackします。
...
Phase 1Aでも、VM、VariableStore、Expression/Format IR、UI変更は行っていません。次はChatレビュー後にPhase 1Bで対応命令の範囲を広げる候補があります。
