...
まだ新しいVMでゲームを動かしてはいません。0Bでは、旧Emueraの実際の読み込み処理と目次を答え合わせし、関数134,652件で予期しない差分0件を確認しました。Phase 1はまだ開始していません。

ゲームのERBが更新されたら、将来は変更されたファイルと影響する依存範囲だけをcacheから外す設計です。設定・parser規則・エンジン版が変わった場合は安全のため全体を作り直します。R1時点では設計のみで、まだ実装していません。
