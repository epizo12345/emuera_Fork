...
{
    private static int RunR0F6G10C1(string root, string outputPath, string mode)
    {
        root = Path.GetFullPath(root);
        outputPath = Path.GetFullPath(outputPath);
        var legacy = mode.StartsWith("Legacy", StringComparison.Ordinal);
        var newGame = mode.EndsWith("NewGame", StringComparison.Ordinal);
        if (!IsCompactProductionProofRoot(root) || File.Exists(outputPath) ||
            mode is not ("LegacyTitle" or "CompactTitle" or "LegacyNewGame" or "CompactNewGame")) return 64;
        try
        {
            ...
            if (!DifferentialDeterminism.Configure(true, NextRuntimeDifferentialSeed,
                ...
                throw new InvalidOperationException("R0-F6G10C1 deterministic setup failed");
            ...
            using (var log = new StreamWriter(new FileStream(Path.ChangeExtension(outputPath, ".init.log"), FileMode.CreateNew)))
                initialized = process.Initialize(log).GetAwaiter().GetResult();
            if (!initialized)
            {
                B1Proof.WriteJson(Path.ChangeExtension(outputPath, ".initialize-failure.json"),
                    window.R0B1Console.R0F6G10C1DisplayModel());
                throw new InvalidOperationException("R0-F6G10C1 initialization failed");
            }
            R0F6G10AHeadlessCapture = true;
            Preload.Clear();
            B1Proof.WriteJson(outputPath, process.R0F6G10C1RunDisplayPath(newGame));
            return 0;
        }
        catch (Exception ex)
        {
